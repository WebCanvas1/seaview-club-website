      ...
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-ink/95 text-white backdrop-blur-md">
        <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {['Venue Hire', "What's On", 'Club Activities', 'About', 'Contact'].map((item) => <button key={item} onClick={() => scrollTo(item === 'Venue Hire' ? 'venue' : item === "What's On" ? 'whats-on' : item === 'Club Activities' ? 'activities' : item.toLowerCase())} className="text-sm text-white/75 transition hover:text-sand">{item}</button>)}
